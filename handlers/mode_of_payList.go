...
func InsertPaymentMode(c *fiber.Ctx) error {
	...
	_, err := db.Pool.Exec(context.Background(), "INSERT INTO mode_of_payments_list (payment_mode) VALUES ($1)", req.PaymentMode)
	if err != nil {
...
func UpdatePaymentMode(c *fiber.Ctx) error {
	...
	_, err = db.Pool.Exec(context.Background(), "UPDATE mode_of_payments_list SET payment_mode = $1 WHERE id = $2", req.PaymentMode, idInt)
	if err != nil {
...
func DeletePaymentMode(c *fiber.Ctx) error {
	...
	_, err = db.Pool.Exec(context.Background(), "DELETE FROM mode_of_payments_list WHERE id = $1", idInt)
	if err != nil {
		log.Println("Error deleting:", err)
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to delete"})
	}
	return c.JSON(fiber.Map{"message": "Payment mode deleted"})
}

func GetPaymentModes(c *fiber.Ctx) error {
	rows, err := db.Pool.Query(context.Background(), "SELECT id, payment_mode FROM mode_of_payments_list")
	if err != nil {
...
func GetPaymentModeByID(c *fiber.Ctx) error {
	...
	var paymentMode string
	err = db.Pool.QueryRow(context.Background(), "SELECT payment_mode FROM mode_of_payments_list WHERE id = $1", idInt).Scan(&paymentMode)
	if err != nil {
